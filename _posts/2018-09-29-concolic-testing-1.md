...
## 部分常见测试策略
事实上，绝大多数动态符号执行测试策略仅执行一遍随机测试，更多的是关注随后符号执行路径的调度，这导致相关算法性能距离[最优调度策略](http://pxzhang.cn/2018/10/08/concolic-testing-2/)有极大的差距。我将使用以下函数作为例子简单阐述几种常见动态符号执行以及符号执行策略。
```
